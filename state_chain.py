...
class IncompleteModification(Exception):
    ...
    def __init__(self, func_names: Iterable[ChainFunction]) -> None:
        self.func_names = func_names

    def __str__(self) -> str:
        return (
            "The following functions have neither been dropped nor added to the "
            "modified chain: %r" % self.func_names
        )
...
class StateChain(Generic[State]):
    ...
    def run(
        ...
        function attribute ``state_chain_exception`` and at the current value of
        ``state.exception``. If ``state.exception`` is ``None``, then we skip
        any function whose exception attribute is :obj:`'required'`, and if
        ...
        Here are some further notes on exception handling:
         ...
         - If a function's exception attribute is :attr:`'optional'`, then that
         ...
         - If ``raise_immediately`` evaluates to ``True`` (looking first at any
           per-call ``raise_immediately`` and then at the instance default),
    ...
    def debug(self, func_ref: ChainFunctionRef) -> ChainFunction:
        ...
        ...     pass
        ...
        >>> algo = StateChain(int, functions=[foo])
        >>> algo.debug(foo)
        <function foo at ...>
        ...
        >>> algo = StateChain(dict, functions=[foo])
        >>> algo.debug('foo')
        <function foo at ...>
        >>> algo.run()                  #doctest: +SKIP
        (Pdb)
